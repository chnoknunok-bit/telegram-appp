...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const promoPercentCode =
        normalizePromoCode(
          req.body?.promoPercentCode ||
          ""
        );

      if (
        !Number.isInteger(amountPt) ||
        amountPt <= 0
      ) {
        return res.status(400).json({
          ok: false,
      ...
      const bonusPoints =
        Math.floor(
          amountPt * percent / 100
        );

      const totalPoints =
        amountPt + bonusPoints;

      const paymentRub =
        Math.ceil(
          amountPt * PT_RUB_RATE
        );
  ...
  async (req, res) => {
    ...
    try {
      ...
      return res.status(400).json({
        ...
        message:
          error.message ||
          "Не удалось получить пополнение"
      });
    }
  }
);
        ...
        await pool.query(
          `
            ...
            SET
              status =
                CASE
                  WHEN status = 'waiting_payment'
                    THEN 'paid_waiting_confirmation'
                  ELSE status
                END,
              paid_at =
                CASE
